...
public class SJFX26_shipei1 extends Fragment {
    private PieChart mTuoxing;
    private List<SJFX> shuju,yes,no;
    int cf=0;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.sjfx_yemian3, container, false);
        return view;
    }
        ...
        new Http().sendResUtil("get_peccancy", jsonObject.toString(), "POST", new Callback() {
            ...
            public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                ...
                try {
                    ...
                    shuju=new Gson().fromJson(jsonObject1.optJSONArray("ROWS_DETAIL").toString(),new TypeToken<List<SJFX>>(){}.getType());
                    yes=new ArrayList<>();
                    no=new ArrayList<>();
                    for (int i=0;i<shuju.size();i++) {
                        SJFX sjfx = shuju.get(i);
    ...
    private void extracted() {
        ...
        pieEntries.add(new PieEntry(no2*100,"无重复违章"));
        pieEntries.add(new PieEntry(no4*100,"有重复违章"));
